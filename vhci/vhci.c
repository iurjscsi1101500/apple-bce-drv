...
static int bce_vhci_bus_suspend(struct usb_hcd *hcd)
{
    ...
    for (i = 0; i < 16; i++) {
        pr_debug("bce_vhci: checking port %d\n", i);
        if (!vhci->port_to_device[i]) {
            pr_debug("bce_vhci: port %d has no device\n", i);
            continue;
        }
        for (j = 0; j != 18; j++) {
            pr_debug("bce_vhci: checking transfer queue %d for port %d\n", j, i);
            if (!(vhci->devices[vhci->port_to_device[i]]->tq_mask & BIT(j))) {
                pr_debug("bce_vhci: transfer queue %d not active for port %d\n", j, i);
                continue;
            }
            pr_debug("bce_vhci: pausing transfer queue %d for port %d\n", j, i);
    ...
    for (i = 0; i < 16; i++) {
        pr_debug("bce_vhci: checking port %d for suspend\n", i);
        if (!vhci->port_to_device[i]) {
            pr_info("bce_vhci: port %d has no device\n", i);
            continue;
        }
        pr_debug("bce_vhci: suspending port %d\n", i);
    ...
    if (status) {
        pr_warn("bce_vhci: failed to suspend controller, error %d\n", status);
        return status;
    }
...
static int bce_vhci_bus_resume(struct usb_hcd *hcd)
{
    ...
    if (status) {
        pr_warn("bce_vhci: failed to resume controller, error %d\n", status);
        return status;
    }

    pr_info("bce_vhci: resume ports\n");
    for (i = 0; i < 16; i++) {
        pr_debug("bce_vhci: checking port %d for resume\n", i);
        if (!vhci->port_to_device[i]) {
            pr_info("bce_vhci: port %d has no device\n", i);
            continue;
        }
        pr_debug("bce_vhci: resuming port %d\n", i);
        bce_vhci_cmd_port_resume(&vhci->cq, i);
    }

    pr_info("bce_vhci: resume endpoints\n");
    for (i = 0; i < 16; i++) {
        pr_debug("bce_vhci: checking port %d for endpoints\n", i);
        if (!vhci->port_to_device[i]) {
            pr_info("bce_vhci: port %d has no device\n", i);
            continue;
        }
        for (j = 0; j != 18; j++) {
            ...
            pr_debug("bce_vhci: resuming transfer queue %d for port %d\n", j, i);
